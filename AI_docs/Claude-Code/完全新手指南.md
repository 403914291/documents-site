...
1. ✅ 修复表格格式（原文表格无法正常渲染）
2. ✅ 优化代码块格式（添加语言标识）
3. ✅ 统一标题层级（使用标准 Markdown 语法）
4. ✅ 优化目录结构（增加快速导航）
5. ✅ 删除冗余内容（重复的引导语和公众号推广）
6. ✅ 优化阅读体验（增加分隔线和章节摘要）
...
| 章节 | 内容 | 适合人群 |
|------|------|---------|
| [第一章](#第一章-认识-claude-code) | 认识 Claude Code | 初学者 |
| [第二章](#第二章-安装与账号配置) | 安装配置 | 首次使用者 |
| [第三章](#第三章-核心概念) | 核心概念 | 所有用户 |
| [第四章](#第四章-快速入门) | 快速入门 | 新手实战 |
| [第五章](#第五章-claudemd-给-claude-持久记忆) | CLAUDE.md 配置 | 进阶用户 |
| [第六章](#第六章-完整命令速查手册) | 命令速查 | 所有用户 |
| [第七章](#第七章-进阶技巧) | 进阶技巧 | 进阶用户 |
| [第八章](#第八章-subagents-独立上下文的专用助手) | Subagents | 高级用户 |
| [第九章](#第九章-hooks-自动化触发器) | Hooks 配置 | 高级用户 |
| [第十章](#第十章-worktree-并行任务隔离) | Worktree | 高级用户 |
| [第十一章](#第十一章-实战-sop) | 实战 SOP | 所有用户 |
| [第十二章](#第十二章-常见错误与避坑指南) | 避坑指南 | 所有用户 |
...
这份指南共 12 章，从最基础的安装配置讲起，深入 Agent Loop 工作原理、上下文管理策略、CLAUDE.md 项目记忆配置，再到 Subagents 专用子代理、Hooks 自动化触发器、Worktree 并行任务隔离等进阶功能，最后提供三套直接可用的实战 SOP。
...
> 
> Claude 越能干，你越要注意：给清晰边界和验证标准，是用好它的关键。
...
这是使用 Claude Code 最重要的认知之一。上下文窗口（约 20 万 token）会消耗你说的每一句话、Claude 读取的每个文件、每个命令的输出，以及压缩后的历史摘要。
...
| **Plan Mode** | Shift+Tab 两次 | 只读分析，不执行任何修改 | 复杂任务先分析再执行 |
| **Auto-Accept** | --dangerously-skip-permissions | 自动批准所有操作 | CI/CD 等完全自动化场景 |

**权限白名单配置**（.claude/settings.json）：
...
```
1. 按 Shift+Tab 两次进入 Plan Mode（提示符会变化）
2. 描述你要做的事，让 Claude 分析影响面和执行步骤
3. 仔细审查输出的规划，可以继续追问某个步骤
4. 确认无误后，按 Shift+Tab 切回 Normal Mode 执行
```
...
| 加测试 | 给 @src/utils/dateUtils.ts 写测试。参考 @src/__tests__/stringUtils.test.ts 的风格。覆盖：空输入、无效格式、时区问题。 |
...
**示例一：代码审查 Subagent**（`.claude/agents/code-reviewer.md`）：
...
**示例二：调试专家 Subagent**（`.claude/agents/debugger.md`）：
...
**示例三：安全审查 Subagent**（`.claude/agents/security-reviewer.md`）：
...
**① 文件修改后自动 Lint**（`.claude/settings.json`）：
...
```
1. 开新 Worktree：claude --worktree feature-[功能名]
2. 需求规格化（5-10 分钟）：访谈 → 生成 SPEC.md
3. 规划（Shift+Tab 进入 Plan Mode）：列出改动文件清单
4. 实现（Normal Mode）：每个类写完立即编译验证
5. 验证（10 分钟）：跑测试 → Subagent 安全审查 → 检查规范
6. 提交：feat(模块): 描述 + 详细说明
```
...
| 问题 | 解决 |
|------|------|
| 完成任务后不执行 /clear 直接开始新任务。Claude 被历史信息干扰，给出不相关甚至矛盾的回答。 | 每次切换任务前执行 /clear，养成习惯。同一问题修正超过两次，直接 /clear 用更精确的提示重新开始。 |

**❌ 提示词太模糊**

| 问题 | 解决 |
|------|------|
| 「帮我优化代码」→ Claude 做了你意想不到的大规模改动，难以回滚。 | 使用黄金提示词公式：目标 + 文件位置 + 验证标准 + 约束条件，四要素缺一不可。 |

**❌ 不先规划就执行大改动**

| 问题 | 解决 |
|------|------|
| 多文件重构直接执行，不了解影响范围，出了问题难以定位。 | 改动超过 3 个文件，先用 Plan Mode（Shift+Tab）看清规划后再执行。 |

**❌ 完全信任 AI 不审查输出**

| 问题 | 解决 |
|------|------|
| 直接 accept 所有改动，不做代码审查，尤其业务逻辑和安全相关部分。 | AI 生成的代码必须经过人工审查，重点看：边界条件、错误处理、权限检查。始终让 Claude 改完后运行测试验证。 |

**❌ 不配置 CLAUDE.md**

| 问题 | 解决 |
|------|------|
| 每次对话都重新解释技术栈、规范、背景，浪费大量 token 和时间。 | 新项目第一件事执行 /init，让 Claude 生成 CLAUDE.md 模板，然后手动补充最重要的规范。 |
...
| 问题 | 解决 |
|------|------|
| CLAUDE.md 几百行，Claude 无法有效注意到关键规则。 | 精简 CLAUDE.md，只留「Claude 最容易弄错的事」。重要规则加「IMPORTANT:」或「YOU MUST:」强调。复杂流程转移到 Skills，按需加载。 |

**❌ Claude 读了太多不必要的文件**

| 问题 | 解决 |
|------|------|
| 调查问题时没限定范围，Claude 扫描整个 repo，上下文快速耗尽。 | 用 @ 精确引用文件而非整个目录。用 Plan Mode 先评估影响面。复杂探索任务改用 Subagent，不污染主会话。 |
