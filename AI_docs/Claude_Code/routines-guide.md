...
> **来源**：基于 Anthropic 官方公告及技术社区报道（量子位、机器之心、钛媒体）

---

## 📰 今日摘要

**2026 年 4 月中旬，Anthropic 正式官宣 Claude Code Routines（研究预览版）**。这是 Claude Code 的里程碑式更新——从此，你的 AI 编程助手不再只是被动响应，而是可以**主动工作**。

**核心价值：**
> 配置好触发条件后，Claude 可以**自主执行任务**——从发现问题到生成修复方案，全程无需人工干预。  
> 你只需要在方便时审核结果即可。

**这就是 Routines 带来的变革。**

本文基于 Anthropic 官方公告、GitHub 仓库文档和早期用户实践，深度解析 Routines 的三种触发方式，并提供完整的实战配置教程。
...
> 💡 这些自动化场景在技术社区已有广泛讨论。开发者们通过配置自动化脚本、CI/CD 工作流等方式，实现类似的 Agent 自动化工作流。Routines 把这些能力整合到了云端，开箱即用。
